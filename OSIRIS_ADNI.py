...
class OSIRIS(object):    
    ...
    def fit_iterate(self, max_iter = 250, pruning_crit = 1e-1, tol = 1e-3, feat_crit = 1e-3, perc = False, verbose = 0, Y_tst = [None], 
        ...
        for i in range(max_iter):            
            ...
            ###############
            Y_pred = self.compute_predictions(X_tst = [None], m_in=[0,1,2,3,4], m_out=5)
            auc_tst = metrics.roc_auc_score(self.Yy, Y_pred)
            print('Auc: ', auc_tst)
            ##############

            # Y_pred = self.X[-1]['mean'][-np.shape(self.Yy)[0]:]
            # # print(Y_pred)
            # hard_pred = np.argmax(Y_pred, axis=1)
            # bal_tst = metrics.balanced_accuracy_score(self.Yy, hard_pred)
            # print('Balanced: ', bal_tst)

            ###############
            # Y_pred = self.predict([0,1],2, self.X[0], self.X[1])
            # auc_tst = metrics.roc_auc_score(self.Yy, Y_pred)
            # print('Auc: ', auc_tst)
            ##############
            
            # if (len(self.L) > 10) and (abs(1 - np.mean(self.L[-10:-1])/self.L[-1]) < tol):
...
#        return (self.sigmoid(x) - 0.5)/(2*x)
    ...
    def update_eta(self):
        ...
        q = self.q_dist
        # q.eta['a'] = (self.hyper.eta_a + 0.5 * np.shape(self.X[-1]['mean'])[1]*self.n_max)/(np.shape(self.X[-1]['mean'])[1]*self.n_max) 
        ...
        #                                              ))/(np.shape(self.X[-1]['mean'])[1]*self.n_max)
...
# =============================================================================
        ...
        if self.method[m_out] == 'reg':   
            #Expectation X
            # mean_x = np.dot(self.G_mean,q.W[m_out]['mean'].T) + q.b[m_out]['mean']
            #Variance X
        ...
        #Multilabel
            ...
            #Expectation X
            # m_x = np.dot(self.G_mean, q.W[m_out]['mean'].T) + q.b[m_out]['mean']  
            #Variance X
            # var_x = q.tau_mean(m_out)**(-1)*np.eye(self.d[m_out]) + np.linalg.multi_dot([q.W[m_out]['mean'], Z_cov, q.W[m_out]['mean'].T])
            ...
            #     p_t[:,d] = self.sigmoid((np.dot(self.G_mean, q.W[m_out]['mean'].T) + np.dot(self.Z_mean, q.V[m_out]['mean'].T))[:,d]*(1+math.pi/8*(q.eta_mean()**(-1)*np.eye(self.d[m_out]) + np.linalg.multi_dot([q.W[m_out]['mean'], G_cov, q.W[m_out]['mean'].T])[d,d] + q.tauz_mean()**(-1)*np.eye(self.d[m_out]) + np.linalg.multi_dot([q.V[m_out]['mean'], Z_cov, q.V[m_out]['mean'].T])[d,d]))**(-0.5))
            #     # p_t[:,d] = self.sigmoid((np.dot(self.G_mean, q.W[m_out]['mean'].T))[:,d]*(1+math.pi/8*(q.eta_mean()**(-1)*np.eye(self.d[m_out]) + np.linalg.multi_dot([q.W[m_out]['mean'], G_cov, q.W[m_out]['mean'].T])[d,d]))**(-0.5))
            return p_t
