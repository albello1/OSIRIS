...
X6 = add_random_nans(X6, args.perc, seed = 123)
...
# =================================================== #
folds_file = '/export/usuarios01/abelenguer/Datos/OSIRIS_exps/Digit/'+str(folds)+'folds_'+database+'.p'
...
if args.imputer == 'None': 
    ...
    imp_mean = SimpleImputer(missing_values=np.nan, strategy='median')
    X6_tr = imp_mean.fit_transform(X6_tr)
    X6_tst = imp_mean.transform(X6_tst)
    ...
    X6_tr, X6_tst = impute_missing_values_VAE(X6_tr,X6_tst)
...
for i in range(np.shape(test_encoded)[1]):
    myModel = OSIRIS_digit.OSIRIS(Kc = 100, Kp = 1, prune = 1, SS_sep = 1, Yy = test_encoded[:,i,np.newaxis], seed_init=args.seed)
    ...
    myModel.fit(X0_t,X1_t,X2_t,X3_t,X4_t,X5_t, Y1_tr, max_iter = 200, Y_tst = Y1_tst, AUC = 0, ACC = 0, verbose = 1)
...
path_save = r'./Results/Imputation/Digit/OSIRIS_'+str(args.seed)+'_'+str(args.fold)+'_'+str(args.perc)+'_'+str(args.imputer)+'.npy'
