...
def standarize_no_nan(matrix):
    scaler = StandardScaler()

    # Aplicar el escalado solo a valores no NaN
    mask_nan = np.isnan(matrix)
    matrix_no_nan = np.where(mask_nan, np.nan, matrix)

    # Ajustar y transformar ignorando los NaN
    matrix_scaled = np.copy(matrix_no_nan)
    for col in range(matrix.shape[1]):
        data_valid = ~mask_nan[:, col]  # Índices de valores no NaN
        if np.any(data_valid):  # Si hay valores válidos en la columna
            matrix_scaled[data_valid, col] = scaler.fit_transform(matrix[data_valid, col].reshape(-1, 1)).flatten()
...
X1 = np.load(r'./Data/Tadpole/view1.npy')
X2 = np.load(r'./Data/Tadpole/view2.npy')
X3 = np.load(r'./Data/Tadpole/view3.npy')
X4 = np.load(r'./Data/Tadpole/view4.npy')
X5 = np.load(r'./Data/Tadpole/view5.npy')
X6 = np.load(r'./Data/Tadpole/view6.npy')
Y = np.load(r'./Data/Tadpole/labels.npy')
...
# =================================================== #
folds_file = './Data/Tadpole/'+str(folds)+'folds_'+database+'.p'
...
if args.model == 'MVFA':
    ...
    X6 = np.vstack((X6_tr, X6_tst))

    print(np.shape(X1)[0],np.shape(X1)[1],np.shape(X2)[1],np.shape(X3)[1],np.shape(X4)[1],np.shape(X5)[1],np.shape(X6)[1])
    ...
    X1 = standarize_no_nan(X1)
    X2 = standarize_no_nan(X2)
    X3 = standarize_no_nan(X3)
    X4 = standarize_no_nan(X4)
    X5 = standarize_no_nan(X5)
    X6 = standarize_no_nan(X6)
    ...
    X5_t = myModel.struct_data(X6, 'reg', 1)
    Y1_tr = myModel.struct_data(train_encoded, 'mult', 0)
    Y1_tst = myModel.struct_data(test_encoded, 'mult', 0)


    myModel.fit(X0_t,X1_t,X2_t,X3_t,X4_t,X5_t, Y1_tr, max_iter = 1000, Y_tst = Y1_tst, AUC = 0, ACC = 0, verbose = 2)

    Y_pred = myModel.compute_predictions(X_tst = [None], m_in=[0,1,2,3,4,5], m_out=6)
    Y_pred_tr = myModel.compute_predictions(X_tst = [None], m_in=[0,1,2,3,4,5], m_out=6, tr = 1)
    ...
    path_guardar = r'./Results/Tadpole/MVFA_'+str(args.seed)+'_'+str(args.fold)+'.npy'
    ...
    imputer.fit(X1_tr)
    X1_tr= imputer.transform(X1_tr)
    X1_tst= imputer.transform(X1_tst)

    imputer = SimpleImputer(missing_values=np.nan, strategy=strategy)
    imputer.fit(X2_tr)
    X2_tr= imputer.transform(X2_tr)
    X2_tst= imputer.transform(X2_tst)

    imputer = SimpleImputer(missing_values=np.nan, strategy=strategy)
    imputer.fit(X3_tr)
    X3_tr= imputer.transform(X3_tr)
    X3_tst= imputer.transform(X3_tst)

    imputer = SimpleImputer(missing_values=np.nan, strategy=strategy)
    imputer.fit(X4_tr)
    X4_tr= imputer.transform(X4_tr)
    X4_tst= imputer.transform(X4_tst)

    imputer = SimpleImputer(missing_values=np.nan, strategy=strategy)
    imputer.fit(X5_tr)
    X5_tr= imputer.transform(X5_tr)
    X5_tst= imputer.transform(X5_tst)

    imputer = SimpleImputer(missing_values=np.nan, strategy=strategy)
    imputer.fit(X6_tr)
    X6_tr= imputer.transform(X6_tr)
    X6_tst= imputer.transform(X6_tst)
    ...
    X3_tr = scaler3.fit_transform(X3_tr)
    X3_tst = scaler3.transform(X3_tst)
    scaler4 = StandardScaler()
    X4_tr = scaler4.fit_transform(X4_tr)
    X4_tst = scaler4.transform(X4_tst)
    scaler5 = StandardScaler()
    X5_tr = scaler5.fit_transform(X5_tr)
    X5_tst = scaler5.transform(X5_tst)
    scaler6 = StandardScaler()
    X6_tr = scaler6.fit_transform(X6_tr)
    X6_tst = scaler6.transform(X6_tst)
    X_tr = [X1_tr, X2_tr,X3_tr,X4_tr,X5_tr,X6_tr]
    X_tst = [X1_tst, X2_tst,X3_tst,X4_tst,X5_tst,X6_tst]
    ...
    path_guardar = r'./Results/Tadpole/PLS_'+str(args.seed)+'_'+str(args.fold)+'.npy'
    with open(path_guardar, "wb") as archivo:
        pickle.dump(results, archivo)
    ...
    path_guardar = r'./Results/Tadpole/MKL_'+str(args.seed)+'_'+str(args.fold)+'.npy'
    with open(path_guardar, "wb") as archivo:
        pickle.dump(results, archivo)
    ...
    path_guardar = r'./Results/Tadpole/CPM_'+str(args.seed)+'_'+str(args.fold)+'.npy'
    with open(path_guardar, "wb") as archivo:
        pickle.dump(results, archivo)
    ...
    X3 = np.concatenate((X3_tr, X3_tst), 0)
    X4 = np.concatenate((X4_tr, X4_tst), 0)
    X5 = np.concatenate((X5_tr, X5_tst), 0)
    X6 = np.concatenate((X6_tr, X6_tst), 0)
    X_views = [X1, X2,X3,X4,X5,X6]
    ...
    results = [soft, Y_tst]
    path_guardar = r'./Results/Tadpole/CSMVIB_'+str(args.seed)+'_'+str(args.fold)+'.npy'
    with open(path_guardar, "wb") as archivo:
        pickle.dump(results, archivo)
    ...
    dims = [[np.shape(X1_tr)[1]], [np.shape(X2_tr)[1]],[np.shape(X3_tr)[1]],[np.shape(X4_tr)[1]],[np.shape(X5_tr)[1]],[np.shape(X6_tr)[1]]]
    views = len(dims)
    ...
    results = [soft, Y_tst]
    path_guardar = r'./Results/Tadpole/TMC_'+str(args.fold)+'_'+str(args.seed)+'.npy'
    with open(path_guardar, "wb") as archivo:
        pickle.dump(results, archivo)
    ...
    path_guardar = r'./Results/Tadpole/DeepIMV_'+str(args.seed)+'_'+str(args.fold)+'.npy'
    with open(path_guardar, "wb") as archivo:
        pickle.dump(results, archivo)
    ...
    for i in range(np.shape(test_encoded)[1]):
        ...
        myModel.fit(X0_t,X1_t,X2_t,X3_t,X4_t,X5_t, Y1_tr, max_iter = 500, Y_tst = Y1_tst, AUC = 0, ACC = 0, verbose = 1)
    ...
    path_guardar = r'./Results/Tadpole/OSIRIS_'+str(args.seed)+'_'+str(args.fold)+'.npy'
    with open(path_guardar, "wb") as archivo:
        pickle.dump(results, archivo)
