...
data_tr = np.loadtxt('/export/usuarios01/abelenguer/Datos/OSIRIS_exps/Sat/sat_trn.csv', delimiter=' ',dtype=int)
data_tst = np.loadtxt('/export/usuarios01/abelenguer/Datos/OSIRIS_exps/Sat/sat_tst.csv', delimiter=' ',dtype=int)

data = np.vstack((data_tr, data_tst))
...
# Here we only select the central pixels of the 3 x 3 pixel matrix for each band (4).
idx_columns_spectral = [16, 17, 18, 19]
X_spectral = X_data[:, idx_columns_spectral]
#And here we calculate the mean value of the other 8 pixels (context) for each band (4).
X_spatial = np.zeros((X_data.shape[0], bands))
for vv in range(bands):
    vista = [cc for cc in range(vv,bands*9,bands) if ((cc < 16) | (cc>19))]
    X_spatial[:,vv] = np.mean(X_data[:,vista], 1)    
X = np.hstack((X_spectral, X_spatial))
...
# =================================================== #
folds_file = '/export/usuarios01/abelenguer/Datos/OSIRIS_exps/Sat/'+str(folds)+'folds_'+database+'.p'
...
if args.model == 'MVFA':
    ...
    myModel = fast_fs_ksshiba_b.SSHIBA(Kc = 100, prune = 0, SS_sep = 1)
    ...
    path_guardar = r'./Results/Sat/MVFA_'+str(args.seed)+'_'+str(args.fold)+'.npy'
    ...
    path_guardar = r'./Results/Sat/PLS_'+str(args.seed)+'_'+str(args.fold)+'.npy'
    with open(path_guardar, "wb") as archivo:
        pickle.dump(results, archivo)
    ...
    path_guardar = r'./Results/Sat/MKL_'+str(args.seed)+'_'+str(args.fold)+'.npy'
    with open(path_guardar, "wb") as archivo:
        pickle.dump(results, archivo)
    ...
    path_guardar = r'./Results/Sat/CPM_'+str(args.seed)+'_'+str(args.fold)+'.npy'
    with open(path_guardar, "wb") as archivo:
        pickle.dump(results, archivo)
    ...
    results = [soft, Y_tst]
    path_guardar = r'./Results/Sat/CSMVIB_'+str(args.seed)+'_'+str(args.fold)+'.npy'
    with open(path_guardar, "wb") as archivo:
        pickle.dump(results, archivo)
    ...
    results = [soft, Y_tst]
    path_guardar = r'./Results/Sat/TMC_'+str(args.fold)+'_'+str(args.seed)+'.npy'
    with open(path_guardar, "wb") as archivo:
        pickle.dump(results, archivo)
    ...
    path_guardar = r'./Results/Sat/DeepIMV_'+str(args.seed)+'_'+str(args.fold)+'.npy'
    with open(path_guardar, "wb") as archivo:
        pickle.dump(results, archivo)
