...
X1 = np.load(r'./Data/Scene15/X1.npy')
X2 = np.load(r'./Data/Scene15/X2.npy')
X3 = np.load(r'./Data/Scene15/X3.npy')
Y = np.load(r'./Data/Scene15/labels.npy')
...
# =================================================== #
folds_file = './Data/Scene15/'+str(folds)+'folds_'+database+'.p'
...
if args.model == 'MVFA':
    ...
    myModel.fit(X0_t,X1_t,X2_t, Y1_tr, max_iter = 500, Y_tst = Y1_tst, AUC = 0, ACC = 0, verbose = 1)

    Y_pred = myModel.compute_predictions(X_tst = [None], m_in=[0,1,2], m_out=3)
    Y_pred_tr = myModel.compute_predictions(X_tst = [None], m_in=[0,1,2], m_out=3, tr = 1)
    ...
    path_guardar = r'./Results/Scene15/MVFA_'+str(args.seed)+'_'+str(args.fold)+'.npy'
    ...
    path_guardar = r'./Results/Scene15/PLS_'+str(args.seed)+'_'+str(args.fold)+'.npy'
    with open(path_guardar, "wb") as archivo:
        pickle.dump(results, archivo)
    ...
    for view_train, view_test in zip(X_tr, X_tst):
        K_train = rbf_kernel(view_train, view_train,gamma=0.5)
        K_test = rbf_kernel(view_test, view_train,gamma=0.5)
    ...
    path_guardar = r'./Results/Scene15/MKL_'+str(args.seed)+'_'+str(args.fold)+'.npy'
    with open(path_guardar, "wb") as archivo:
        pickle.dump(results, archivo)
    ...
    path_guardar = r'./Results/Scene15/CPM_'+str(args.seed)+'_'+str(args.fold)+'.npy'
    with open(path_guardar, "wb") as archivo:
        pickle.dump(results, archivo)
    ...
    results = [soft, Y_tst]
    path_guardar = r'./Results/Scene15/CSMVIB_'+str(args.seed)+'_'+str(args.fold)+'.npy'
    with open(path_guardar, "wb") as archivo:
        pickle.dump(results, archivo)
    ...
    dims = [[np.shape(X1_tr)[1]], [np.shape(X2_tr)[1]],[np.shape(X3_tr)[1]]]
    views = len(dims)
    ...
    results = [soft, Y_tst]
    path_guardar = r'./Results/Scene15/TMC_'+str(args.fold)+'_'+str(args.seed)+'.npy'
    with open(path_guardar, "wb") as archivo:
        pickle.dump(results, archivo)
    ...
    path_guardar = r'./Results/Scene15/DeepIMV_'+str(args.seed)+'_'+str(args.fold)+'.npy'
    with open(path_guardar, "wb") as archivo:
        pickle.dump(results, archivo)
    ...
    for i in range(np.shape(test_encoded)[1]):
        ...
        myModel.fit(X0_t,X1_t,X2_t, Y1_tr, max_iter = 100, Y_tst = Y1_tst, AUC = 0, ACC = 0, verbose = 1)
    ...
    path_guardar = r'./Results/Scene15/OSIRIS_'+str(args.seed)+'_'+str(args.fold)+'.npy'
    with open(path_guardar, "wb") as archivo:
        pickle.dump(results, archivo)
