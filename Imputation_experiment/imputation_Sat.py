...
X1_tr = add_random_nans(X1_tr, args.perc, seed = 123)
X2_tr = add_random_nans(X2_tr, args.perc, seed = 123)

X1_tst = add_random_nans(X1_tst, args.perc, seed = 123)
X2_tst = add_random_nans(X2_tst, args.perc, seed = 123)
...
for i in range(np.shape(test_encoded)[1]):
    myModel = OSIRIS_sat.OSIRIS(Kc = 100, Kp = 1, prune = 1, SS_sep = 1, Yy = test_encoded[:,i,np.newaxis], seed_init=args.seed)
...
path_save = r'./Results/Imputation/Sat/OSIRIS_'+str(args.seed)+'_'+str(args.fold)+'_'+str(args.perc)+'_'+str(args.imputer)+'.npy'
