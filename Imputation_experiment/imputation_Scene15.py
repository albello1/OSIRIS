...
for i in range(np.shape(test_encoded)[1]):
    myModel = OSIRIS_scene15.OSIRIS(Kc = 100, Kp = 1, prune = 1, SS_sep = 1, Yy = test_encoded[:,i,np.newaxis])
    ...
    myModel.fit(X0_t,X1_t,X2_t, Y1_tr, max_iter = 50, Y_tst = Y1_tst, AUC = 0, ACC = 0, verbose = 1)
...
path_save = r'./Results/Imputation/Scene15/OSIRIS_'+str(args.seed)+'_'+str(args.fold)+'_'+str(args.perc)+'_'+str(args.imputer)+'.npy'
