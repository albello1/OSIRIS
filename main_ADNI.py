...
# =================================================== #
folds_file = './Data/ADNI/'+str(folds)+'folds_'+database+'.p'
...
if args.model == 'MVFA':
    ...
    myModel.fit(X0_t,X1_t,X2_t,X3_t,X4_t, Y1_tr, max_iter = 500, Y_tst = Y1_tst, AUC = 0, ACC = 0, verbose = 1)
    ...
    path_guardar = r'./Results/ADNI/MVFA_'+str(args.seed)+'_'+str(args.fold)+'.npy'
    ...
    scaler2 = StandardScaler()
    X3_tr = scaler2.fit_transform(X3_tr)
    X3_tst = scaler2.transform(X3_tst)
    scaler2 = StandardScaler()
    X4_tr = scaler2.fit_transform(X4_tr)
    X4_tst = scaler2.transform(X4_tst)
    scaler2 = StandardScaler()
    X5_tr = scaler2.fit_transform(X5_tr)
    X5_tst = scaler2.transform(X5_tst)
    
    X_tr = [X1_tr, X2_tr, X3_tr, X4_tr, X5_tr]
    X_tst = [X1_tst, X2_tst, X3_tst, X4_tst, X5_tst]
    ...
    path_guardar = r'./Results/ADNI/PLS_'+str(args.seed)+'_'+str(args.fold)+'.npy'
    with open(path_guardar, "wb") as archivo:
        pickle.dump(results, archivo)
    ...
    path_guardar = r'./Results/ADNI/MKL_'+str(args.seed)+'_'+str(args.fold)+'.npy'
    with open(path_guardar, "wb") as archivo:
        pickle.dump(results, archivo)
    ...
    path_guardar = r'./Results/ADNI/CPM_'+str(args.seed)+'_'+str(args.fold)+'.npy'
    with open(path_guardar, "wb") as archivo:
        pickle.dump(results, archivo)
    ...
    results = [soft, Y_tst]
    path_guardar = r'./Results/ADNI/CSMVIB_'+str(args.seed)+'_'+str(args.fold)+'.npy'
    with open(path_guardar, "wb") as archivo:
        pickle.dump(results, archivo)
    ...
    dims = [[np.shape(X1_tr)[1]], [np.shape(X2_tr)[1]], [np.shape(X3_tr)[1]], [np.shape(X4_tr)[1]], [np.shape(X5_tr)[1]]]
    views = len(dims)
    ...
    results = [soft, Y_tst]
    path_guardar = r'./Results/ADNI/TMC_'+str(args.fold)+'_'+str(args.seed)+'.npy'
    with open(path_guardar, "wb") as archivo:
        pickle.dump(results, archivo)
    ...
    path_guardar = r'./Results/ADNI/DeepIMV_'+str(args.seed)+'_'+str(args.fold)+'.npy'
    with open(path_guardar, "wb") as archivo:
        pickle.dump(results, archivo)
    ...
    myModel = OSIRIS.OSIRIS(Kc = 100, Kp = 1, prune = 1, SS_sep = 1, Yy = Y_tst, seed_init=args.seed)
    X0_t = myModel.struct_data(X1, 'reg', 1)
    X1_t = myModel.struct_data(X2, 'reg', 1)
    X2_t = myModel.struct_data(X3, 'reg', 1)
    X3_t = myModel.struct_data(X4, 'reg', 1)
    ...
    path_guardar = r'./Results/ADNI/OSIRIS_'+str(args.seed)+'_'+str(args.fold)+'.npy'
    with open(path_guardar, "wb") as archivo:
        pickle.dump(results, archivo)
